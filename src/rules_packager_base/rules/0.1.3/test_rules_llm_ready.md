...
checksum: 2f4d4cc6c8b440cb57e0f3ee980e71805fd9b754d7aa3ae316c0cc288d399fb9
---
...
- **String equality success conditions:** A success condition may be authored as `{n} = <TEXT>`.
  - **Subjective (operator observation):** If `{n}` is entered by an operator as an observation (free text, may be empty), then `{n} = <TEXT>` is validated by an operator decision (PASS/FAIL/SKIP), not by string equality.
    ...
    - Store the operator decision as `PASS`/`FAIL`/`SKIP` under `verdicts[<criterion_id>]` (criterion-keyed). Measurements remain under `measurements[...]` and are linked from criteria via `ref` (single) or `refs` (multi).
    - Use `operator_judgment(meas_id, target, log)` from `rules_packager_base` when available.
...
- `steps`: array of step objects. Each object has:
  - `text`: copy the step line verbatim, then apply the JSON encoding rules below.
...
- **JSON encoding (mandatory)**: the emitted JSON text MUST be ASCII-only and MUST NOT contain any `\\uXXXX` escape sequences.
  - Replace non-ASCII characters with ASCII equivalents:
    ...
    - Micro `u`
    - Ohm `ohm`
  - If the procedure contains other non-ASCII characters, replace them with a reasonable ASCII equivalent.
...
- Controller `id` may be free-form; generated code naming must be derived from controller `subtype`, not from `id`.
...
**Example (JSON output; truncated steps list):**
```json
{
  "name": "CAP-CHARGE-HOLD-001",
  "description": "",
