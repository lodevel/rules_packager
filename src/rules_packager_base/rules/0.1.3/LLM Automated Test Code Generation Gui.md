...
### Existing equipment configuration is immutable

When updating an existing test script, do not change existing equipment configuration values.

- Keep existing VISA addresses, COM ports, baud rates, timeouts, channel assignments, and similar equipment configuration exactly as written.
- Add new configuration variables only when required for newly added equipment or newly required parameters.
- If a requested change requires different equipment configuration, stop and ask the user instead of modifying it.
...
Generate controller code names from `subtype`, not `id`.

- For each controller subtype, use `<NORMALIZED_SUBTYPE><N>` in first-appearance order, starting at 1.
- Example: `fncore-mockup` -> `FNCORE_MOCKUP1`, `FNCORE_MOCKUP2`
- Generated parameter prefixes use the uppercase form: `FNCORE_MOCKUP1_PORT`, `FNCORE_MOCKUP1_BAUD`, `FNCORE_MOCKUP1_TIMEOUT`
- Generated Python variable names use the lowercase form: `fncore_mockup1`, `fncore_mockup2`
